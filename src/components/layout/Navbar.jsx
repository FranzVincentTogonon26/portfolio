    ...
    >
        ...
        <div className='flex items-center justify-between'>
         {/* Logo */}
          <div className='flex items-center gap-4'>
            <Code className='w-6 g-6 text-primary' />
            <button 
      ...
      >
          <div className='bg-black/95 backdrop-blur-lg border-t border-white/10 px-5 py-6 space-y-3'>
            {NAV_LINKS.map((link) => (
              <button
                className={`block w-full text-left px-4 py-3 rounded-lg font-medium transition-all duration-300
                    ${ activeSection === link.id ? 'text-white bg-white/10' : 'text-white/70 hover:text-white hover:bg-white/50' }
                  `}
                key={link.id}
                onClick={() => handleNavClick(link.id)}
              >
            ...
            <button
              onClick={() =>  handleNavClick('contact')}
              className='w-full px-7 py-3.5 bg-white text-[#212121] font-medium text-base rounded-lg border border-white hover:bg-white/90 transition-all duration-300 mt-2'
            >
              Hire Me
            </button>
